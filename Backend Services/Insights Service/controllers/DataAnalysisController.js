...
async function ExpQueries(experimentId, InstanceID){
  try {
    ...
    WITH eng AS 
    (SELECT ResearcherId, raw.StudyId, raw.ExperimentId, raw.InstanceId, FLOOR(Timestamp) AxisTime, IF(Item =0,0,1) AxisEngagement, GameMode, CharacterType
    FROM
      (SELECT * 
    ...
    SELECT ResearcherId, StudyId, ExperimentId, InstanceId, AxisTime, AxisEngagement, BreakdownType, BreakdownName
    FROM
        ...
        GROUP BY  1,2,3,4,5,7,8) all_eng
    GROUP BY 1,2,3,4,5,7,8
    ORDER BY AxisTime
...
async function StudyQueries() {
  try {
    ...
    WITH eng AS 
    (SELECT ResearcherId, raw.StudyId, FLOOR(Timestamp) AxisTime, IF(Item =0,0,1) AxisEngagement, GameMode, CharacterType
    FROM
      (SELECT * 
      FROM yarr.raw_data_view  
    ...
    SELECT ResearcherId, StudyId, AxisTime, AxisEngagement, BreakdownType, BreakdownName
    FROM
